...
It is always useful to have a quick glance at data set structure to find out how information it contains is structured. Knowledge of the structure is important, because it allows us later to filter out desired information very precisely based on criteria to limit specific dimensions, i.e. variables.
...
Before we go any further and start with the analysis of covid data we will replicate the above queries using the `dplyr` functions. First, to illustrate how easy it is to do the column selection with `dplyr` we'll select `countriesAndTerritories` and continentExp` from our `covid_ecdc` data.
...
covid_ecdc %>%
  ...
  tally()
```
If you do not have to manipulate and do the engineering work with DBs, but you need to access it to obtain data for the analysis, you might find it easier to do it all using the `dplyr`. It is intuitive and therefore easier to write. You cannot deny that `dplyr`'s version looks neater and easier to write.  
...
If you are interested in learning more about creating maps in R check the online version of the book [Geocomputation with R](https://geocompr.robinlovelace.net). [Chapter 8: The Making maps with R](https://geocompr.robinlovelace.net/adv-map.html) provides an  to follow overview on using the `tmap` and other packages for creating beautiful maps in R.

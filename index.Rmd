...
The practice has shown that for a public health intervention to be successful the effort from the governmental authorities need to be put into:

- identifying the infected and bringing them to treatments 
- locate and quarantine all those who had contact with the infected 
- sterilise environmental pathogens 
- promote use of mask and social distancing
- release to the public number of infected and deaths on a daily basis through open data
...
```
  We use `tbl()` to define a table as if it was part of the R work-space, and specify as the function's arguments the connection object and the name of the table in the database.
```{r, eval=FALSE}
...
[European Centre for Disease Prevention and Control](https://www.ecdc.europa.eu/en/publications-data/download-todays-data-geographic-distribution-covid-19-cases-worldwide) provides daily update of new reported cases of COVID-19 by country worldwide. The downloadable data file is updated daily and contains the latest available public data on COVID-19. Each row/entry contains the number of new cases reported per country and per day (with a lag of a day).
...
url2ecdc <- url2 <- "https://www.ecdc.europa.eu/sites/default/files/documents/COVID-19-geographic-disbtribution-worldwide-2020-11-11.xlsx"
...
Let's see what tables do we have in our database?
```{r}
dbListTables(SQLcon)
```
...
and see how many entries are there for the UK, using a `where` clause.
```{r}
dbFetch(
  dbSendQuery(SQLcon,
               "Select continentExp, count(*) as Count  
...
What is nice for people who are rusty with SQL, and who don't feel like learning both SQL and R, is that we can do all of this using the `diplyr` package in R.
...
Before we go any further and start with the analysis of covid data we will replicate the above queries using the `dplyr` functions. First, to illustrate how easy is to do the columns selections with `dplyr` we'll select `countriesAndTerritories` and continentExp` from our `covid_ecdc` data.
...
First query was the counts of entries for each continent.
```{r}
covid_ecdc %>% 
...
We will focus our analysis on European countries and select them from our `covid_world` data, saving it all as `covid_eu` data frame object as this data needs tidying and wrangling and we do not want to limit ourself on using only `dplyr` functions in R. 
...
As we would like to be able to plot this time series of the second derivatives $F^{’’}(x)$ for any country we will create functions that woill allow us for extracting a country from the `covid_eu` data and for plotting it as a time series. 
...
We will present the same information this time using the line plot integrating interactivity in display of the information by using the `ggplotly()` function.
...
Sometimes it might be useful to present a several plots next to each other. To do this in R we apply the `plot_grid()` function from the `cowplot` package.  
...
The following plot enables us to observe the change to the acceleration in relaion to the governmental measurments.
...
Let us see how does those figures compare with other countries in particular France and Germany.
...
We can present total number of infections for each month. As the numbers are heigh we are going to "control" the way the values on the `y` access are going to appear. 
...
We will create choropleth in which we will colour the EU countries according to the most current value of cumulative number for 14 days of COVID-19 cases per 100000. To do this we will use the shape file onto which we will superimpose this value as a colour of the polygon, i.e. country. 
...
With the `tmap` package, thematic maps can be generated with great effectiveness presenting several layers of information. The syntax is similar to the one adopted in `ggplot`. Motivation and the explanation of this package has been proposed and published in [tmap: Thematic Maps in R](https://www.jstatsoft.org/article/view/v084i06) article. 

If you're interested learning more about creating maps in R check the online version of the book [Geocomputation with R](https://geocompr.robinlovelace.net). [Chapter 8: The Making maps with R](https://geocompr.robinlovelace.net/adv-map.html) of the book provides easy to follow overview on using the `tmap` and other packages for crating beautiful maps in R.
